...
- [x] Dockerfile
- [x] Конвертация FB2 → TXT, EPUB, PDF
- [x] Оглавление и изображения в читалке
...
weasyprint >= 60.0            # опционально, для конвертации в PDF
```

Для weasyprint нужны системные пакеты (Debian/Ubuntu):

```bash
# Debian 12 Bookworm / Ubuntu 22+
apt-get install libpango-1.0-0 libpangoft2-1.0-0 libharfbuzz0b fonts-dejavu-core
```
...
- Скачивание и конвертация FB2 → TXT, EPUB, PDF через кнопку ⬇

**Онлайн-чтение FB2 (кнопка «📖 Read»):**
- Регулировка размера шрифта, ночной режим
- Прогресс-бар с маркерами закладок; клик — переход к позиции
- Оглавление (TOC) — боковая панель, закрывается кликом вне или кнопкой ✕
- Панель закладок — сохранение текущей позиции, переход по закладке
- Скачивание в разных форматах через кнопку ⬇ (FB2 / TXT / EPUB / PDF)
- Управление клавиатурой: `←` / `→` или `PageUp` / `PageDown` — перелистывание; `↑` / `↓` — плавная прокрутка
- Управление тапами на мобильном: левые и правые 10% экрана — зоны перелистывания (верх 70% → вперёд, низ 30% → назад)
